...
const ripple: Move = $derived({
  ...
  intent: "/assets/intents/defendDebuff.png",
  name: "Ripple",
});
...
const timeEater: Enemy = $derived({
  flowchart: `
    flowchart-elk TB
      A(${reverberate.effect}<img src=${reverberate.intent} class="h-10 object-contain" /><span class="text-xs">Max Consecutive: 2\n45%</span>)
      B(${headSlam.effect}<img src=${headSlam.intent} class="h-10 object-contain" /><span class="text-xs">Max Consecutive: 1\n35%</span>)
      C(${ripple.effect}<img src=${ripple.intent} class="h-10 object-contain" /><span class="text-xs">Max Consecutive: 1\n20%</span>)
